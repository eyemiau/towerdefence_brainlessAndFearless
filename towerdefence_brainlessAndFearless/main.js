...
function setupCanvas() {
  // Ограничиваем DPR до 2 — этого достаточно для чёткости на Retina MacBook,
  // и не создаёт лишней нагрузки на GPU по сравнению с реальным dpr=2/3
  ...
  // Сброс трансформации перед новым scale — критично при повторных resize
  // (например, при переключении между MacBook и внешним Windows-монитором через один браузер)
  ...
  // Тайл считается по меньшей стороне — одинаковая логика для 16:9 (Windows)
  // и 16:10 (MacBook), поле всегда квадратное 40x40 и не обрезается
  tileSize = Math.max(
    8,
    Math.floor(Math.min(logicalW / COLS, logicalH / ROWS)),
  );

  // Центрируем поле — на широких 16:9 экранах появятся поля по бокам,
  // на более "квадратных" MacBook — сверху/снизу. Это ожидаемо и корректно.
