...
class Enemy {
  ...
  update(dt) {
    ...
    if (dist <= speedPixels) {
      ...
      if (this.role === "digger") {
        // Сканируем дистанцию до ближайшей башни
        let minTowerDist = Infinity;
        for (let t of towers) {
          let dist = Math.hypot(this.col - t.col, this.row - t.row);
        ...
        if (minTowerDist <= SCAN_RADIUS) {
          ...
          // Читаем вектор уклонения
          vector = currentTile.diggerVector; 
          
          // ИСПРАВЛЕНИЕ: Если уклонение завершено (вектор null), переключаемся на базу
          if (!vector) {
        ...
        if (vector) {
          ...
          // ... [стандартная логика для остальных мобов]
...
class Pathfinder {
  ...
  calculateAllyField() {
    ...
    let activeLanes = [];
    for (let e of enemies) {
      if (e.role !== "digger") { // Игнорируем землекопов, фокус на ударной силе
    ...
    if (activeLanes.length > 0) {
      // Если есть угроза, целью становятся именно те участки дороги, где идут враги
      ...
      // Если врагов нет (перерыв между волнами), союзники стягиваются к точке спавна
      distances[this.spawnTile.col][this.spawnTile.row] = 0;
      heap.push({ tile: this.spawnTile, dist: 0 });
    }

    // 3. Распространяем волну (Flood Fill) только по проходимым тайлам
    while (!heap.isEmpty()) {
      ...
      for (let n of this.getNeighbors(current)) {
        // КРИТИЧНО: Союзники должны передвигаться строго по дорогам.
  ...
  calculateFlowField(isForDigger) {
    ...
    for (let t of towers) {
          // ИСПОЛЬЗУЕМ ПОЛНЫЙ РАДИУС: Землекоп должен "видеть" реальную границу атаки
            ...
            for (let dr = -searchOffset; dr <= searchOffset; dr++) {
              ...
              if (nc >= 0 && nc < COLS && nr >= 0 && nr < ROWS) {
                ...
                if (dist <= fearRadius) {
                  if (isForDigger) {
                    // ГРАДИЕНТНЫЙ ШТРАФ:
                    // Если клетка находится на самом краю зоны поражения (внешнее кольцо толщиной ~1 клетка)
                    if (dist > fearRadius - 1.2) {
                      // Легкий штраф: Землекоп пожертвует безопасностью ради сокращения пути
                      this.grid.cells[nc][nr].penalty += 1.5; 
                    } else {
                      // Глубоко в радиусе поражения: избегать любой ценой
...
let distances = Array(COLS).fill().map(() => Array(ROWS).fill(Infinity));
    ...
    if (isForDigger) {
      // ИНТЕЛЛЕКТУАЛЬНАЯ КОПКА: Все главные дороги становятся целями.
      // Начинаем дистанцию с уровня их опасности (penalty).
      for (let c = 0; c < COLS; c++) {
        for (let r = 0; r < ROWS; r++) {
    ...
    // ... финальный цикл в calculateFlowField ...
    for (let c = 0; c < COLS; c++) {
      for (let r = 0; r < ROWS; r++) {
        let tile = this.grid.cells[c][r];
        if (tile === this.baseTile || (!isForDigger && tile.type === "grass")) continue;
        
        // ИСПРАВЛЕНИЕ: minDist стартует с текущей дистанции тайла, а не с Infinity.
        ...
        if (minNeighbor) {
          ...
          // Если сосед с меньшей дистанцией не найден, явно очищаем вектор
