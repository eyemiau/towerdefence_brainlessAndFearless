 ...
 * + Динамический поиск пути (Flow Fields & Object Pool Audio)
 */
...
const MAX_TOWERS = 15;
const enemies = [], towers = [], allies = [], particles = [];
...
function spawnGoldText(x, y, amount) {
  particles.push({
    x, y, life: 0.8, maxLife: 0.8, isText: true, amount,
    update(dt) {
      this.y -= 25 * dt;
      this.life -= dt;
    },
...
// ==========================================
if (typeof window.playSfx !== "function") window.playSfx = function () {};
if (typeof window.MUSIC === "undefined") window.MUSIC = { stop: function () {}, play: function () {} };

function safePlaySfx(name) {
  try { playSfx(name); } catch (err) {}
}
function safeStopMusic() {
  try { MUSIC.stop(); } catch (err) {}
}
...
class Enemy {
  ...
  update(dt) {
    ...
    if (engagedAlly) {
      this.facingAngle = Math.atan2(engagedAlly.y - this.y, engagedAlly.x - this.x);
      this.meleeTimer += dt;
    ...
    if (this.role === "siege") {
      ...
      if (this.targetTower) {
        this.facingAngle = Math.atan2(this.targetTower.y - this.y, this.targetTower.x - this.x);
        if (this.timeSinceAttack >= this.cooldown) {
          this.targetTower.hp -= this.damage;
          this.timeSinceAttack = 0;
          this.isFiring = true;
          this.trail = { x2: this.targetTower.x, y2: this.targetTower.y, life: 0.2, maxLife: 0.2 };
        }
        return;
      }
    }
    ...
    if (dist <= speedPixels) {
      ...
      if (this.role === "digger") {
        ...
        for (let t of towers) {
          let distT = Math.hypot(this.col - t.col, this.row - t.row);
          if (distT < minTowerDist) minTowerDist = distT;
        }

        const SCAN_RADIUS = 10;
        let vector;

        if (minTowerDist <= SCAN_RADIUS) {
          ...
          vector = currentTile.diggerVector; 
          
          if (!vector) {
             vector = currentTile.vector;
          }
  ...
  draw(ctx) {
    ...
    grad.addColorStop(0, this.hitFlash > 0 ? "#ffffff" : lighten(this.color, 55));
    ...
    drawRoundRect(ctx, this.x - hpW / 2, this.y - tileSize - r * 0.5, hpW, 4, 2);
    ctx.fillStyle = "rgba(0,0,0,0.5)";
    ctx.fill();
    const hpPct = Math.max(0, this.hp / this.maxHp);
    const hpColor = hpPct > 0.5 ? "#2ecc71" : hpPct > 0.2 ? "#f39c12" : "#e74c3c";
    drawRoundRect(ctx, this.x - hpW / 2 + 1, this.y - tileSize - r * 0.5 + 1, (hpW - 2) * hpPct, 2, 1);
...
class Ally {
  ...
  update(dt) {
    ...
    if (engagedEnemy) {
      this.facingAngle = Math.atan2(engagedEnemy.y - this.y, engagedEnemy.x - this.x);
      this.attackTimer += dt;
  ...
  draw(ctx) {
    ...
    ctx.beginPath();
    ctx.moveTo(this.x, this.y);
    ctx.lineTo(this.x + Math.cos(this.facingAngle) * spearLen, this.y + Math.sin(this.facingAngle) * spearLen);
    ctx.stroke();
    ...
    ctx.fillStyle = "rgba(0,0,0,0.5)";
    ctx.fill();
    drawRoundRect(ctx, this.x - hpW / 2 + 1, this.y - tileSize - r * 0.5 + 1, (hpW - 2) * Math.max(0, this.hp / this.maxHp), 2, 1);
...
class Tower {
  ...
  update(dt) {
    if (this.buildAnim < 1) this.buildAnim = Math.min(1, this.buildAnim + dt * 4);
    this.timer += dt;
    this.isFiring = false;
    ...
    if (this.target && (this.target.hp <= 0 || Math.hypot(this.x - this.target.x, this.y - this.target.y) > this.range)) {
      this.target = null;
    }
    if (!this.target) {
      for (let e of enemies) {
    ...
    if (this.target && this.timer >= this.cooldown) {
      ...
      this.trail = { x1: muzzleX, y1: muzzleY, x2: this.target.x, y2: this.target.y, life: 0.12, maxLife: 0.12 };
      spawnBurst(muzzleX, muzzleY, "#ffd54f", 4, 0.6);
      safePlaySfx(this.type === "Rapid" ? "shootRapid" : this.type === "Sniper" ? "shootSniper" : "shootBasic");
    }
  }
  ...
  draw(ctx) {
    ...
    ctx.beginPath();
    ctx.ellipse(this.x, this.y + tSize * 0.42, tSize * 0.45, tSize * 0.18, 0, 0, Math.PI * 2);
    ctx.fillStyle = "rgba(0,0,0,0.35)";
    ctx.fill();

    const grad = ctx.createRadialGradient(this.x - tSize * 0.15, this.y - tSize * 0.15, tSize * 0.05, this.x, this.y, tSize * 0.7);
    grad.addColorStop(0, lighten(this.color, 45));
    grad.addColorStop(1, darken(this.color, 15));
    drawRoundRect(ctx, this.x - tSize / 2 + 2, this.y - tSize / 2 + 2, tSize - 4, tSize - 4, 6);
    ctx.fillStyle = grad;
    ctx.fill();
    ...
    ctx.fillStyle = "white";
    const pipSize = tileSize * 0.13, startX = this.x - pipSize * this.level;
    for (let i = 0; i < this.level; i++) {
      ctx.fillRect(startX + i * pipSize * 2, this.y - tSize / 2 + 4, pipSize, pipSize);
    }

    drawRoundRect(ctx, this.x - tSize / 2 + 2, this.y + tSize / 2 - 7, tSize - 4, 4, 2);
    ctx.fillStyle = "rgba(0,0,0,0.5)";
    ctx.fill();
    drawRoundRect(ctx, this.x - tSize / 2 + 3, this.y + tSize / 2 - 6, (tSize - 6) * Math.max(0, this.hp / this.maxHp), 2, 1);
...
function drawFortressStructure(ctx, x, y, size) {
  ...
  drawRoundRect(ctx, x + size * 0.08, y + size * 0.12, size * 0.84, size * 0.8, size * 0.05);
  ...
  const merlonCount = 6, merlonW = (size * 0.84) / merlonCount;
  for (let i = 0; i < merlonCount; i++) {
    if (i % 2 === 0) {
      ctx.fillRect(x + size * 0.08 + i * merlonW, y + size * 0.06, merlonW * 0.85, size * 0.08);
    }
  }

  const towerR = size * 0.11;
  const towerPositions = [
    [x + size * 0.12, y + size * 0.16], [x + size * 0.88, y + size * 0.16],
    [x + size * 0.12, y + size * 0.84], [x + size * 0.88, y + size * 0.84],
  ];
  ...
  const gateW = size * 0.22, gateH = size * 0.3;
  const gateGrad = ctx.createLinearGradient(cx - gateW / 2, y + size * 0.92 - gateH, cx + gateW / 2, y + size * 0.92);
...
function drawPortalStructure(ctx, x, y, size) {
  ...
  const outer = size * 0.42, inner = size * 0.32, core = size * 0.24;
  ...
  const baseGrad = ctx.createLinearGradient(cx - outer, cy - outer, cx + outer, cy + outer);
  ...
  for (const [ox, oy] of cornerOffsets) {
    const bx = cx + ox * outer, by = cy + oy * outer;
    const tipX = cx + ox * (outer + size * 0.1), tipY = cy + oy * (outer + size * 0.1);
    ctx.beginPath();
  ...
  for (let side = 0; side < 4; side++) {
    for (let i = 0; i < notches; i++) {
      ...
      let px, py;
      if (side === 0) { px = cx - outer + t * outer * 2; py = cy - outer; }
      else if (side === 1) { px = cx - outer + t * outer * 2; py = cy + outer; }
      else if (side === 2) { px = cx - outer; py = cy - outer + t * outer * 2; }
      else { px = cx + outer; py = cy - outer + t * outer * 2; }
      ctx.beginPath();
...
function drawAnimatedStructureFX(ctx, time) {
  ...
  const fcx = fortX + fortSize / 2, fcy = fortY + fortSize / 2;
  ...
  ctx.beginPath();
  ctx.moveTo(0, 0);
  ctx.quadraticCurveTo(fortSize * 0.15, -3 + wave * 0.3, fortSize * 0.28, 2 + wave);
  ctx.lineTo(fortSize * 0.28, fortSize * 0.14 + wave);
  ctx.quadraticCurveTo(fortSize * 0.15, fortSize * 0.11 + wave * 0.3, 0, fortSize * 0.16);
  ctx.closePath();
  ctx.fillStyle = "#3498db";
  ctx.fill();
  ...
  const scx = spawnX + spawnSize / 2, scy = spawnY + spawnSize / 2;

  const pPulse = 0.5 + Math.sin(time * 3) * 0.25;
  const glowSize = spawnSize * 0.42;
  const pGlow = ctx.createRadialGradient(scx, scy, 0, scx, scy, spawnSize * 0.5);
  ...
  drawRoundRect(ctx, scx - glowSize, scy - glowSize, glowSize * 2, glowSize * 2, spawnSize * 0.03);
  ctx.fillStyle = pGlow;
  ctx.fill();
  ...
  for (let side = 0; side < 4; side++) {
    for (let i = 0; i < notchCount; i++) {
      ...
      let rx, ry;
      if (side === 0) { rx = scx - perim + t * perim * 2; ry = scy - perim; }
      else if (side === 1) { rx = scx - perim + t * perim * 2; ry = scy + perim; }
      else if (side === 2) { rx = scx - perim; ry = scy - perim + t * perim * 2; }
      else { rx = scx + perim; ry = scy - perim + t * perim * 2; }
      ctx.beginPath();
  ...
  if (Math.random() < 0.3) {
    particles.push(new Particle(scx + (Math.random() - 0.5) * spawnSize * 0.5, scy + (Math.random() - 0.5) * spawnSize * 0.5, "#c084fc", 0.3));
  }
}
...
class Grid {
  ...
  createMap() {
    for (let c = 0; c < COLS; c++) {
      ...
      for (let r = 0; r < ROWS; r++) {
        let tile = {
          col: c, row: r, type: "grass", isBuildable: true, hasTower: false, towerRef: null,
          baseCost: Infinity, currentCost: Infinity, distance: Infinity,
          vector: null, diggerVector: null, allyVector: null, isMainRoad: false,
        };

        if ((c >= 3 && c <= 36 && r >= 3 && r <= 4) || (r >= 3 && r <= 36 && c >= 3 && c <= 4)) {
          tile.type = "road"; tile.baseCost = 1; tile.isBuildable = false; tile.isMainRoad = true;
        }
        if ((c >= 3 && c <= 36 && r >= 35 && r <= 36) || (r >= 3 && r <= 36 && c >= 35 && c <= 36)) {
          tile.type = "road"; tile.baseCost = 1; tile.isBuildable = false; tile.isMainRoad = true;
        }
        if (Math.abs(c + r - 39) <= 1 && c >= 3 && c <= 36) {
          tile.type = "road"; tile.baseCost = 0.8; tile.isBuildable = false; tile.isMainRoad = true;
        }
        if (c >= 2 && c <= 5 && r >= 34 && r <= 37) {
          tile.type = "base"; tile.isBuildable = false; tile.baseCost = 1;
        }
        if (c >= 34 && c <= 37 && r >= 2 && r <= 5) {
          tile.type = "spawn"; tile.isBuildable = false; tile.baseCost = 1;
        }
  ...
  cacheBackground(ctx) {
    ...
    for (let i = 0; i < 900; i++) {
      const x = rnd() * logicalW, y = rnd() * logicalH;
      const c = gameGrid.cells[Math.floor((x - offsetX) / tileSize)]?.[Math.floor((y - offsetY) / tileSize)];
    ...
    for (let c = 0; c < COLS; c++) {
      for (let r = 0; r < ROWS; r++) {
        ...
        if (t.type === "spawn") {
          ...
          if (c === 2 && r === 34) drawFortressStructure(ctx, x, y, tileSize * 4);
        } else if (t.type === "road") {
...
class Pathfinder {
  ...
  calculateAllyField() {
    ...
    // 1. Сканируем карту: определяем активные линии (дороги с врагами)
    let activeLanes = [];
    for (let e of enemies) {
      if (e.role !== "digger") { 
        let cell = this.grid.cells[e.col][e.row];
        if (cell && cell.type === "road") {
          activeLanes.push(cell);
        }
      }
    }

    // 2. Устанавливаем цели для союзников
    if (activeLanes.length > 0) {
      for (let laneCell of activeLanes) {
        if (distances[laneCell.col][laneCell.row] !== 0) {
          distances[laneCell.col][laneCell.row] = 0;
          heap.push({ tile: laneCell, dist: 0 });
        }
      }
    } else {
    ...
    // 3. Распространяем волну (Flood Fill)
    while (!heap.isEmpty()) {
    ...
    // 4. Генерируем allyVector для движения
    for (let c = 0; c < COLS; c++) {
      for (let r = 0; r < ROWS; r++) {
        ...
        if (minNeighbor) {
          tile.allyVector = { x: minNeighbor.col - tile.col, y: minNeighbor.row - tile.row };
        }
      }
    }
  }
  ...
  calculateFlowField(isForDigger) {
      ...
      for (let dc = -searchOffset; dc <= searchOffset; dc++) {
        for (let dr = -searchOffset; dr <= searchOffset; dr++) {
          ...
          if (nc >= 0 && nc < COLS && nr >= 0 && nr < ROWS) {
            ...
            let dist = Math.hypot(nc - centerCol, nr - centerRow);

            if (dist <= fearRadius) {
              if (isForDigger) {
                if (dist > fearRadius - 1.2) {
                  this.grid.cells[nc][nr].penalty += 1.5; 
                } else {
    ...
    if (isForDigger) {
      for (let c = 0; c < COLS; c++) {
        for (let r = 0; r < ROWS; r++) {
          let cell = this.grid.cells[c][r];
          if (cell.type === "road" && cell.isMainRoad) {
            distances[c][r] = cell.penalty;
            heap.push({ tile: cell, dist: cell.penalty });
          }
        }
      }
    } else {
    ...
    for (let c = 0; c < COLS; c++) {
      for (let r = 0; r < ROWS; r++) {
        ...
        if (minNeighbor) {
          ...
          if (isForDigger) tile.diggerVector = null;
          else tile.vector = null;
        }
      }
    }
  }
...
class WaveManager {
  ...
  startNextWave() {
    ...
    if (this.waveNumber >= 5)
      for (let i = 0; i < Math.floor(this.waveNumber / 4); i++) this.enemiesToSpawn.push("Siege");
...
function setupCanvas() {
  ...
  tileSize = Math.max(8, Math.floor(Math.min(logicalW / COLS, logicalH / ROWS)));
  ...
  try {
    ...
    if (mouse.col !== -1 && mouse.row !== -1 && !selectedTower) {
      ...
      for (let c = 0; c < size; c++) {
        for (let r = 0; r < size; r++) {
          let nc = mouse.col + c, nr = mouse.row + r;
          if (nc >= COLS || nr >= ROWS || !gameGrid.cells[nc][nr].isBuildable || gameGrid.cells[nc][nr].hasTower)
            canBuild = false;
        }
      }
    ...
    console.error("Ошибка игрового цикла:", err);
  }
