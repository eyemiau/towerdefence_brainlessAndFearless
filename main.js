 ...
 * FIX: защита от падения игрового цикла при отсутствии/ошибке звуковой системы
 */
...
// (защищает игровой цикл от падения, если playSfx/MUSIC не подключены)
// ==========================================
...
class Enemy {
  ...
  draw(ctx) {
    if (this.isDigging) {
      ctx.beginPath();
      ctx.arc(this.x, this.y, this.radius * tileSize * 2, 0, Math.PI * 2);
      ctx.fillStyle = "rgba(243, 156, 18, 0.4)";
      ctx.fill();
    }
    ctx.beginPath();
    ctx.arc(this.x, this.y, tileSize * this.radius, 0, Math.PI * 2);
    ctx.fillStyle = this.color;
    ctx.fill();

    const hpW = tileSize * 0.8;
    ctx.fillStyle = "#e74c3c";
    ctx.fillRect(this.x - hpW / 2, this.y - tileSize, hpW, 3);
    ctx.fillStyle = "#2ecc71";
    ctx.fillRect(
      this.x - hpW / 2,
      this.y - tileSize,
      hpW * Math.max(0, this.hp / this.maxHp),
      3,
    );
...
class Ally {
  ...
  draw(ctx) {
    ...
    ctx.fill();
    ctx.fillStyle = "#e74c3c";
    ctx.fillRect(this.x - tileSize * 0.4, this.y - tileSize, tileSize * 0.8, 3);
    ctx.fillStyle = "#3498db";
    ctx.fillRect(
      this.x - tileSize * 0.4,
      this.y - tileSize,
      tileSize * 0.8 * Math.max(0, this.hp / this.maxHp),
      3,
    );
  }
}
...
class Tower {
  ...
  draw(ctx) {
    ...
    ctx.fillStyle = "red";
    ctx.fillRect(this.x - tSize / 2 + 2, this.y + tSize / 2 - 6, tSize - 4, 3);
    ctx.fillStyle = "#2ecc71";
    ctx.fillRect(
      this.x - tSize / 2 + 2,
      this.y + tSize / 2 - 6,
      (tSize - 4) * Math.max(0, this.hp / this.maxHp),
      3,
    );
